        ...
        webSocket {
            ...
            coroutineScope {
                val serverUUID = call.parameters["serverId"]?.parseServerUUIDOrNull()
                    ?: return@coroutineScope close(
                        CloseReason(
                            CloseReason.Codes.PROTOCOL_ERROR,
                            "Could not parse server UUID"
                        )
                    )
                ...
                try {
                    val currentRunsFlow = websocketAPIService.getAllCurrentRunsFlow(serverUUID)
                        ?: return@coroutineScope close(
                            CloseReason(
                                CloseReason.Codes.PROTOCOL_ERROR,
                                "Could not find runs for server with UUID $serverUUID"
                            )
                        )
                    ...
                    withContext(Dispatchers.IO) {
                        call.application.environment.log.trace(
                            "Collecting current runs updates for server {}",
                            serverUUID
                        )
            ...
            webSocket("/console") {
                ...
                coroutineScope {
                    ...
                    val outputJob = launch(Dispatchers.IO) {
                        ...
                        call.application.environment.log.trace("Ending console websocket output job for runner {}, run {}", runnerUUID, runUUID)
                    }
                    ...
                    val inputJob = launch(Dispatchers.IO) {
                        ...
                        call.application.environment.log.trace("Ending console websocket input job for runner {}, run {}", runnerUUID, runUUID)
                    }
